...
/// Adds a finalizer record into an `MaskReservation` kind of resource. If the finalizer already exists,
...
pub async fn add(client: Client, name: &str, namespace: &str) -> Result<MaskReservation, Error> {
...
/// Removes all finalizers from an `MaskReservation` resource. If there are no finalizers already, this
...
///
/// Note: Does not check for resource's existence for simplicity.
pub async fn delete(client: Client, name: &str, namespace: &str) -> Result<MaskReservation, Error> {
    let api: Api<MaskReservation> = Api::namespaced(client, namespace);
    let finalizer: Value = json!({
        "metadata": {
            "finalizers": null
